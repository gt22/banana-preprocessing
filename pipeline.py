...
class Pipeline:
    ...
    def run(self, x, y) -> Optional[CleanScoreData]:
        if self.preproc is not None:
            ...
            for train_id, val_id in self.preproc.get_split(x, y):
                ...
                val_pred = self._predict(ind(x, val_id, reset_index=True))
            ...
            return self.scorer.score(y, self._predict(x))
    ...
    def run_predict(self, x):
        x_proc = self.preproc.preproc(x, fit=False)
        return self._predict(x_proc)
